...
def csv_to_shape(data_path, csv_full_path, shape_full_path):
    ...
    df_burn = df.query("True_Label == 1") # Query rows at True Label = 1

    # Join the latitude and longitude points into one column
    df_burn["coordinates"] = df_burn["Latitude"].apply(str)+ ", " +df_burn["Longitude"].apply(str)
    ...
    df_burn.to_csv(csv_full_path, index=False, encoding='utf-8')
