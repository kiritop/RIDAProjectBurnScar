...
def preprocess(data_paths, random_state, drop_columns):
    """
    ...
    if drop_columns is None:
        drop_columns = [0, 1, 11, 12, 13, 14, 15, 16, 17, 18, 19]
    ...
    ## Export Normalize as Pickle
    ### Specify the path where you want to save the model
    save_path = 'D:\Work\Code งาน\Lab-docker\RIDA\Export Model\26-03-2024'
    ...
    # Efficient renaming
    df_rename = df_nor.rename(columns=dict(zip(df_nor.columns, Available_Features)))
        
    df_rename = pd.concat([df_rename, label], axis=1, sort=False)
    print(df_rename)
    print("Number of All Unburn class: ", len(df_class_0_all.index))
    print("Number of Unburn class: ", len(df_class_0.index))
    print("Number of Burn class: ", len(df_class_1.index))
...
def knn(X, Y, cv, cv_test_scores, std_test_scores):
    ...
    plt.text(optimal_k, score_cv[_index] + 0.01, 
...
def random_forest(X, Y, cv, random_forest_estimator_num, random_state=42, 
    ...
    for max_depth in max_depth_range:
        rdf = RandomForestClassifier(n_estimators=random_forest_estimator_num, max_depth=max_depth, 
    ...
    plt.axvline(x=optimal_max_depth, color='red', linestyle='--')
    plt.text(optimal_max_depth, scores_cv[_index] + 0.01, 
             f'Optimal Depth = {optimal_max_depth}', color='red')
    plt.show()
    ...
    rdf_model = RandomForestClassifier(n_estimators=random_forest_estimator_num, 
                                       max_depth=optimal_max_depth, random_state=random_state)
    rdf_model.fit(X, Y)
...
def support_vector_machine(X, Y, cv, cv_test_scores=None, std_test_scores=None):
    ...
    plt.text(optimal_c, scores_cv[_index] + 0.01, f'Optimal C = {optimal_c}', color='red')
    plt.show()
...
def multi_layer_perceptron(X, Y, cv, cv_test_scores=None, std_test_scores=None):
    ...
    for hidden_layer_size in hidden_layer_sizes:
        ...
        for train_index, test_index in kfold.split(X, Y):
            ...
            clf = MLPClassifier(hidden_layer_sizes=(hidden_layer_size,), activation='relu', max_iter=500, 
                                random_state=42)
...
def print_classification_report(model_defs, X, Y, cv):
    ...
    directory = (r"D:\Work\Code งาน\Lab-docker\RIDA\Results\Classification Report\Train")
    filename = "22-03-2024 Ban Pa Na Mae Suay Train Classification Report.csv"
...
def save_optimal_model(knn_model, rdf_model, lgb_model, gnb_model, svm_model, mlp_model, X, Y, path):
    """
...
def main():
    ...
    data_paths = [
        r'D:\Work\Code งาน\Lab-docker\RIDA\RIDA_CSV\Train\26-03-2024\[Laos] Ban Xay\2019\T48QUH_20190412T033739_B12_burn_data.csv',
        r'D:\Work\Code งาน\Lab-docker\RIDA\RIDA_CSV\Train\26-03-2024\[Myanmar] Mong Hsu\2019\T47QME_20190329T035709_B12_burn_data.csv',
        r'D:\Work\Code งาน\Lab-docker\RIDA\RIDA_CSV\Train\26-03-2024\[Thailand] Ban Pa Na Mae Sauy\2019\T47QNB_20190326T034539_B12_burn_data.csv',
        r'D:\Work\Code งาน\Lab-docker\RIDA\RIDA_CSV\Train\26-03-2024\[Vietnam] Pleiku\2019\T48PYA_20191229T031129_B12_burn_data.csv'
    ]
    random_state = 42
    drop_columns = None
    df_rename = preprocess(data_paths, random_state, drop_columns)
    ...
    path = (r'D:\Work\Code งาน\Lab-docker\RIDA\Export Model\26-03-2024') # Path to export pickle files
    save_optimal_model(knn_model, rdf_model, lgb_model, gnb_model, svm_model, mlp_model, X, Y, path)
