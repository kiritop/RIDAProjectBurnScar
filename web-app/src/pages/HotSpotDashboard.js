        ...
        <Grid container spacing={3}>
            ...
            <Card sx={{ borderRadius: 3, overflow: "hidden", height:'400px' }} variant="outlined">
              <CardContent>
                ...
                <Typography  variant="subtitle1" color="text.secondary">
                  {provinceText != 'ALL' ? provinceText +', '+ countryText : countryText}  Hot spot (  {format(new Date(dateValue.start),'MMM dd yyyy')} - {format(new Date(dateValue.end),'MMM dd yyyy')} )
                </Typography>
                <Box height={50}/>
          ...
          <Grid item xs={12} md={12}>
            <Box sx={{ borderRadius: 3, overflow: "hidden", flex: 1}}>
                <MUIDataTable
                  title={<h3>Hot spot Ranking {format(new Date(dateValue.start),'MMM dd yyyy')} - {format(new Date(dateValue.end),'MMM dd yyyy')}</h3>}
                  data={tableData}
                  columns={columns}
                  options={options}
                />
